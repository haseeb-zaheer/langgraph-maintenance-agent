...
if [[ "$status" -eq 124 ]]; then
  write_script_failure_report "script_timeout" "The maintenance run exceeded LANGGRAPH_MAINTENANCE_TIMEOUT_SECONDS."
  exit "$status"
fi
...
if [[ "$status" -ne 0 ]]; then
  ...
  if ! find reports -maxdepth 1 -type f -name '*-failure.md' -mmin -5 | grep -q .; then
    write_script_failure_report "script_failure" "The maintenance command exited with status ${status}."
  fi
  exit "$status"
fi
